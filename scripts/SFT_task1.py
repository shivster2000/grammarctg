...
parser.add_argument("--preprossed_dataset_file", type=str, default="SFT_data_multi.jsonl", help="Preprocessed annotated corpus for fine-tuning. Default: %(default)s")
parser.add_argument("--n_test", type=int, default=256, help="Number of items to evaluate on. Default: %(default)s")
...
output_dir = f'{args.checkpoint_dir}llama3_FT_multi'
...
if not os.path.exists(f"../data/{args.preprossed_dataset_file}"):
    ...
    multi_constraints = [
        {"context": extract[0],
         "response": extract[1],
         "constraints": [nr for nr in nrs if idx in all_hit_indices[nr]],
...
train_test_split = dataset.train_test_split(test_size=args.n_test if len(dataset)>500 else 0.2)
train_dataset, test_dataset = train_test_split['train'], train_test_split['test']
unconstrained = test_dataset.map(helpers.get_generation_prompt,
                                 fn_kwargs={"apply_chat_template": tokenizer.apply_chat_template,
                                            "unconstrained": True})

def compute_metrics(eval_preds, n=args.n_test, datasets={"test": test_dataset}, eval_quality=False, ground_truth=False, do_sample=False):
    results = {}
    for name, ds in datasets.items():
        ...
        if ground_truth:
            ...
            outputs = models.generate(model, tokenizer, subset['prompt'], do_sample=do_sample, batch_size=8, eos_token_id=terminators)
...
all_metrics.update(compute_metrics([], datasets={"base": test_dataset}))
all_metrics.update(compute_metrics([], datasets={"unconstrained": unconstrained}))
print(all_metrics)
...
all_metrics.update(compute_metrics([], datasets={"train": train_dataset, "test": test_dataset}, do_sample=True))
all_metrics.update(compute_metrics([], datasets={"train_no_sampling": train_dataset, "test_no_sampling": test_dataset}, do_sample=False))
